...
export default function Header({ today, realToday, selectedDateStr, onDateChange, isCustomDate }) {
      ...
      <div className="max-w-screen-xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs text-slate-500 font-medium whitespace-nowrap">Simulation Date</label>
            <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-all ${
              isCustomDate
                ? 'bg-amber-500/10 border-amber-500/40'
                : 'bg-white/[0.04] border-white/[0.08]'
            }`}>
              <Calendar size={13} className={isCustomDate ? 'text-amber-400' : 'text-blue-400'} />
              <input
                type="date"
          ...
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08]">
            <div className={`w-1.5 h-1.5 rounded-full ${isCustomDate ? 'bg-amber-400' : 'bg-emerald-400'} animate-pulse`} />
            <span className={`text-sm font-medium ${isCustomDate ? 'text-amber-300' : 'text-slate-300'}`}>
              {isCustomDate ? '(Simulated) ' : ''}{dateStr}
            </span>
          </div>
        </div>
      </div>
